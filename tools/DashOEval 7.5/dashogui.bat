...
:rcCheckDone
set DASHO_CMD_LINE_ARGS=%options%
:args
if ""%1""=="""" goto start
...
:start
javaw 2>NUL
if ERRORLEVEL==9009 goto launchJar
start /B javaw -Xms%memoryStart% -Xmx%memoryMax% %javaOptions% -cp "%DASHO_HOME%DashOPro.jar" DashOProGui %DASHO_CMD_LINE_ARGS%
goto end
:launchJar
start /D "%DASHO_HOME%" cmd /c launchJar.bat
:end
endlocal